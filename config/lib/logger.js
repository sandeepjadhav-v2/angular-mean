...
logger.getLogOptions = function getLogOptions() {
  const conf = _.clone(config, true);
  const configFileLogger = conf.log.fileLogger;

  if (!_.has(conf, 'log.fileLogger.directoryPath') || !_.has(conf, 'log.fileLogger.fileName')) {
...
logger.getLogFormat = function getLogFormat() {
  ...
  if (!_.includes(validFormats, format)) {
    ...
    if (process.env.NODE_ENV !== 'test') {
      console.log();
      console.log(
        chalk.yellow(`Warning: An invalid format was provided. The logger will use the default format of "${format}"`)
      );
      console.log();
    }
  }
